    ...
    <script>
                ...
                if (h._hs) {
                    ...
                    h.clickHandlerFunc = function() {
                        ...
                        if (hs.type === 'nav' && hs.target_scene) {
                            if (confirm('Shko te "' + (hs.target_scene.title?.sq || 'skena') + '" për të parë?')) {
                                const ts = scenes.find(s => s.id === hs.target_scene.id);
                                if (ts) { selectScene(ts.id); } else { msg('Skena nuk u gjet'); }
                            }
        ...
        async function saveNewScene() {
            ...
            if (!uploadData.success) {
                msg('Gabim gjatë ngarkimit të imazhit: ' + (uploadData.errors ? Object.values(uploadData.errors).flat().join(', ') : uploadData.message));
                return;
            }
            ...
            if (sceneRes.success) {
                ...
                msg('Gabim: ' + (sceneRes.errors ? Object.values(sceneRes.errors).flat().join(', ') : sceneRes.message));
            }
        }
    </script>
</body>
